...
var L_EnterValidPhoneNumber_Text = "一部の電話番号を削除できません。";
...
var L_altnumber_Text = "この代わりの番号をダイヤルする(<u>D</u>):";
var L_altnumber01_Text = "* この代わりの番号をダイヤルする(<u>D</u>):";
var L_outsideline_Text = "この外線発信番号を含める(<u>O</u>):";
var L_outsideline01_Text = "* この外線発信番号を含める(<u>O</u>):";
var L_callwaiting_Text = "キャッチホンを無効にする(<u>C</u>):";
var L_callwaiting01_Text = "* キャッチホンを無効にする(<u>C</u>):";
...
function Enable_TelBtn_ActMode()
{
    if (InActivation_AppMode())
    {
        ...
        var L_RemindMeLater_Text = "この作業を実行しないで続けるには、<b>[後で通知する]</b> をクリックしてください。";
        g.RemindMeLaterText.innerHTML = L_RemindMeLater_Text;
        var L_RemindMeLater01_Text = " 後で通知する(<u>M</u>) ";
...
function NewToobusy_LoadMe()
{
    ...
    var L_tbstr1_Text = "ライセンス認証の手続きとユーザー登録を行うためのこの番号に接続できません。";
    var L_tbstr2_Text = "ライセンス認証の手続きを行うためのこの番号に接続できません。";
    var L_tbstr3_Text = "ユーザー登録を行うためのこの番号に接続できません。";

    var L_tbstr4_Text = "コンピュータのライセンス認証の手続きとユーザー登録を行わないで先に進むには、<br><b>[省略]</b> をクリックしてください。";
    var L_tbstr5_Text = "コンピュータのライセンス認証の手続きを行わないで続けるには、<br><b>[省略]</b> をクリックしてください。";
    var L_tbstr6_Text = "コンピュータをユーザー登録しないで先に進むには、<br><b>[省略]</b> をクリックしてください。";
...
function DesktopPbErr_LoadMe()
{
    ...
    if (g_DoActivation)
    {
        if  ((g_PostToMS) && (g_PostToOEM))
        {
            L_DeskMainPBerrMsg_Text        = "オンラインでのユーザー登録とライセンス認証の手続きは、お住まいの国または地域では現在利用できません。インターネット接続ではない方法でコンピュータをユーザー登録するには、%1 に連絡してください。インターネットに接続するか、Microsoft に電話して Windows のライセンス認証の手続きを行えます。";
        ...
        else if ((g_PostToMS) && (!g_PostToOEM))
        {
            ...
            GetPageTitle();
            L_DeskMainPBerrMsg_Text        = "オンラインでのユーザー登録とライセンス認証の手続きは、お住まいの国または地域では現在利用できません。インターネットに接続するか、Microsoft に電話して Windows のライセンス認証の手続きを行えます。Microsoft に Windows をユーザー登録するにはインターネット接続が必要です。";
            g.Desktoppberr_par1.innerText  = L_DeskMainPBerrMsg_Text;
        }
        else if ((!g_PostToMS) && (g_PostToOEM))
        {
            L_DeskMainPBerrMsg_Text        = "オンラインでのライセンス認証の手続きは、お住まいの国または地域では現在利用できません。インターネットに接続するか、Microsoft に電話して Windows のライセンス認証の手続きを行えます。インターネット接続ではない方法でコンピュータをユーザー登録するには、%1 に連絡してください。";
            g.Desktoppberr_par1.innerText  = ApiObj.FormatMessage(L_DeskMainPBerrMsg_Text, L_DeskpbErrOEMName_Text);
        }
        else
        {
            ...
            L_DeskMainPBerrMsg_Text        = "オンラインでのライセンス認証の手続きは、お住まいの国または地域では現在利用できません。インターネットに接続するか、Microsoft に電話して Windows のライセンス認証の手続きを行えます。";
            g.Desktoppberr_par1.innerText  = L_DeskMainPBerrMsg_Text;
        }
    }
    else
    {
        if ((!g_PostToMS) && (g_PostToOEM))
        {
            ...
            GetPageTitle();
            L_DeskMainPBerrMsg_Text        = "オンラインでのユーザー登録は、お住まいの国または地域では現在利用できません。インターネット接続ではない方法でコンピュータをユーザー登録するには、%1 に連絡してください。";
            g.Desktoppberr_par1.innerText  = ApiObj.FormatMessage(L_DeskMainPBerrMsg_Text, L_DeskpbErrOEMName_Text);
        }
    }
